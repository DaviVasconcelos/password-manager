...
6. Testes de ViewModels: desacoplar os ViewModels de tipos WinUI para torná-los testáveis e cobrir com xUnit/FluentAssertions. **Decisão**: **incluir**. **Plano detalhado**: `docs/plans/06-testes-viewmodels.md` — 8 etapas incrementais para agente de IA: 6.1 abstrações (`IClipboardService`, `ITimer`/`ITimerFactory`, `IIdiomaProvider`), 6.2 refatorar `VaultViewModel` (`VaultViewModel.cs:30,76,240` → `ITimer`+`IClipboardService`), 6.3 refatorar `SettingsViewModel` (`SettingsViewModel.cs:98` → `IIdiomaProvider`), 6.4 criar `tests/PasswordManager.UI.Tests` (`net8.0-windows`, `PasswordManager.slnx:8`) + fakes (`FakeClipboardService`, `FakeTimer`), 6.5 testes `VaultViewModel` (filtros, CRUD, timers, clipboard, banner), 6.6 testes `SettingsViewModel`/`ItemEditorViewModel`/`UnlockViewModel`, 6.7 CI/TRX, 6.8 limpeza. Ver `docs/plans/06-testes-viewmodels.md:2` para critérios de pronto e riscos.
