...
- Single test project: `dotnet test tests/PasswordManager.Domain.Tests/PasswordManager.Domain.Tests.csproj` (xUnit + FluentAssertions). Same for `tests/PasswordManager.Infrastructure.Tests/PasswordManager.Infrastructure.Tests.csproj`.
...
- Clean Architecture: `src/PasswordManager.Domain` (no deps), `Application` (interfaces only), `Infrastructure` (CryptoService + EF Core persistence), `UI` (WinUI 3 scaffold only).
...
- Storage is a single encrypted JSON blob (ADR 0003): `serialize whole Vault ->` `encrypt with AES-256-GCM ->` upsert one SQLite row. Implemented by `VaultRepository` (Infrastructure). Search/filter happens in memory after decrypt.
...
- Implemented: Domain entities + 34 domain tests; Infrastructure `CryptoService` + `VaultRepository` (SQLite/EF Core, ADR 0003) + 34 infrastructure tests. Stub: Application interfaces only, UI is `App`+`MainWindow` scaffold. `tests/PasswordManager.Application.Tests` has no test files. No EF migration yet (tool `dotnet-ef` not installed; schema is created with `EnsureCreated` in tests).
