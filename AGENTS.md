...
- Clean Architecture: `src/PasswordManager.Domain` (no deps), `Application` (interfaces + `VaultSessionService`), `Infrastructure` (CryptoService + EF Core persistence), `UI` (WinUI 3 scaffold only).
...
- Implemented: Domain entities + 34 domain tests; Infrastructure `CryptoService` + `VaultRepository` (SQLite/EF Core, ADR 0003) + 40 infrastructure tests; Application `VaultSessionService` (criar/desbloquear/trancar/trocar senha mestra/salvar) + fakes + 17 application tests. Stub: UI is `App`+`MainWindow` scaffold. No EF migration yet (tool `dotnet-ef` not installed; schema is created with `EnsureCreated` in tests).
